    ...
    <AppLayout>
      <motion.div className="space-y-6" variants={stagger} initial="hidden" animate="show">
        ...
        <motion.div variants={stagger} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <motion.div variants={fadeUp}><KPICard title="CAMs This Month" value={String(assessments.length)} icon={FileText} color="orange" /></motion.div>
          <motion.div variants={fadeUp}><KPICard title="Active Fraud Flags" value={String(totalFlags)} icon={AlertTriangle} color="red" pulse={totalFlags > 0} /></motion.div>
          <motion.div variants={fadeUp}><KPICard title="Total Exposure" value={totalLoanRequested > 0 ? formatINR(totalLoanRequested) : '₹0'} icon={TrendingDown} color="green" /></motion.div>
          <motion.div variants={fadeUp}><KPICard title="Assessments" value={String(assessments.length)} icon={Clock} color="teal" /></motion.div>
        </motion.div>

        {/* Main grid */}
        <motion.div variants={fadeUp} className="grid grid-cols-1 lg:grid-cols-10 gap-6">
          <motion.div
                      ...
                      <TableBody>
                        {assessments.map((a, idx) => (
                          <motion.tr
                            ...
                            whileHover={{ backgroundColor: 'hsl(18 100% 60% / 0.06)', x: 4 }}
                            className={`cursor-pointer border-b border-border/20 border-l-4 border-l-transparent transition-colors ${idx % 2 === 1 ? 'bg-secondary/15' : ''}`}
            ...
            >
              <Card className="border-border/30">
                ...
                <CardContent className="space-y-3 max-h-64 overflow-y-auto">
                  ...
                  {fraudAlerts.slice(0, 5).map((f, idx) => (
                    <motion.div
                      key={f.id}
                      initial={{ opacity: 0, x: 12 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: idx * 0.06 }}
                      whileHover={{ x: 4, backgroundColor: 'hsl(0 0% 100% / 0.04)' }}
                      className="flex items-start gap-2.5 text-xs border-b border-border/20 pb-3 last:border-0 last:pb-0 -mx-2 px-2 py-1.5 rounded-lg cursor-pointer"
                    >
                      ...
                      <div className="space-y-1.5 mt-3">
                        {fraudTypesData.map((f, i) => (
                          <motion.div
                            key={f.name}
                            whileHover={{ x: 4 }}
                            className="flex items-center justify-between text-[11px] py-0.5 cursor-default"
                          >
        ...
        <motion.div variants={fadeUp} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <motion.div whileHover={{ scale: 1.005 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}>
            <Card className="border-border/30">
              <CardHeader className="pb-2">
